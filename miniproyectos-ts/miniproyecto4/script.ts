...
const formatos = [
  {
    nombre: "parrafo",
    cantidadMaxima: 100,
    funcion: (tipoTexto: string, cantidad: number) => {
      return Array.from({ length: cantidad }, () =>
        obtenerTexto(tipoTexto)
      ).join("<br>");
    },
  },
  {
    ...
    funcion: (tipoTexto: string, cantidad: number) => {
      ...
      if (cantidad > palabras.length) {
        const repetidas = Array.from(
          { length: Math.ceil(cantidad / palabras.length) },
          () => obtenerTexto(tipoTexto)
        )
          .flat()
          .slice(0, cantidad);
  ...
  },
  {
    ...
    funcion: (tipoTexto: string, cantidad: number) => {
      ...
      if (cantidad > letras.length) {
        let repetidas = Array.from(
          { length: Math.ceil(cantidad / letras.length) },
          () => obtenerTexto(tipoTexto)
        )
          .flat()
          .slice(0, cantidad);
...
const errorMensaje = document.querySelector(".error") as HTMLElement;

function obtenerTexto(tipoTexto: string) {
  const contenido = texto[tipoTexto as keyof typeof texto];
...
boton?.addEventListener("click", function (e: Event) {
  ...
  const tipoTexto = (document.getElementById("tipoTexto") as HTMLSelectElement)
    .value;
  const tipoFormato = (
    document.getElementById("tipoValor") as HTMLSelectElement
  ).value;
